...
const App = () => {
  ...
  const addTask = () => {
    axios
      .post(`${BASE_URL}task/addtask`, { title, description },authHeader)
      .then((response) => {
        setTasks([...tasks, response.data.task]);
        console.log("New task added:", response.data.task);
  ...
  const toggleCompletion = (id, completed) => {
    axios
      .put(`${BASE_URL}task/${id}`, { completed },authHeader)
      .then(() => {
        setTasks(
  ...
  // Delete a task
   const deleteTask =async (id) => {
    await axios
      .delete(`${BASE_URL}task/${id}`,authHeader)
